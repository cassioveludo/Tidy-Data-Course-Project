...
After reading the data set homepage and opening the zip file, we realize that the data we need is spread over several data sets. There are two main data collections containing the results for the two sets (test and train). There is also data sets storing labels for the six the activities and the names of the variables observed during the experiment. We need to merge all that information on a single data.frame in order to proceed with the exercise.

We have already downloaded the and unzipped the file, so now we proceed to merge the data set. Before, let's just make sure our working directory is the same where data is stored in our computer
```{r}
list.files()

```

The list.files() function lists the files we have in our current working directory. Note that we need to access the files inside the unzipped file 'UCI HAR Dataset'. After we adjust the working directory we can aplly the read.table() function to read the several data sets we have downloaded and store them in objects so we can merge all of them in a single Final Data Frame

### Test Group

Let's start working with the data sets from test group.
```{r}
#setwd("~/Desktop/datasciencecoursera/Getting_Cleaning_Data/Tidy-Data-Course-Project/UCI HAR Dataset")
